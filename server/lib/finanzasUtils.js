...
const RATE_SOURCES = new Set(['BCV', 'EURO', 'USDT', 'MANUAL'])
...
export function normalizeMovement(input = {}) {
  ...
  const tasaVes = Number(input.tasaVes ?? input.tasa_ves)
  const fuenteTasa = String(input.fuenteTasa ?? input.fuente_tasa ?? '').trim().toUpperCase()
  ...
  if (!concepto || concepto.length > 180) throw new RangeError('concepto obligatorio o demasiado largo')
...
export function summarizeRows(rows = []) {
  ...
  for (const row of rows) {
    const total = Number(row.total_ves) || 0
    const key = `${row.tipo}:${row.categoria}`
    const item = categories.get(key) || { tipo: row.tipo, categoria: row.categoria, total_ves: 0, movimientos: 0 }
    item.total_ves = roundMoney(item.total_ves + total)
    item.movimientos += Number(row.movimientos) || 0
    categories.set(key, item)
    summary.movimientos += Number(row.movimientos) || 0
    if (row.tipo === 'ingreso') summary.ingresos_ves = roundMoney(summary.ingresos_ves + total)
    if (row.tipo === 'egreso') summary.egresos_ves = roundMoney(summary.egresos_ves + total)
  }
  summary.balance_ves = roundMoney(summary.ingresos_ves - summary.egresos_ves)
  summary.categorias = [...categories.values()].sort((a, b) => b.total_ves - a.total_ves)
  return summary
}
