...
export function drawSimplifiedHeader(doc, logoData, config, rightTitle = '', customBgColor = null, customTextColor = null) {
  ...
  if (logoData) {
    try { doc.addImage(logoData, 'PNG', MARGIN + 4, 0.75, 10.5, 10.5, 'HEADER_LOGO', 'FAST') } catch (_) { /* logo opcional */ }
  }
...
export function drawPremiumHeader({
  ...
  // Logo
  if (logoData) {
    try { doc.addImage(logoData, 'PNG', MARGIN + 11, 3, 34, 34) } catch (_) { /* logo opcional */ }
  }
